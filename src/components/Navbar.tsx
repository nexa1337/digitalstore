...
export const Navbar: React.FC = () => {
  ...
  return (
    <div className="fixed top-4 z-50 w-full px-4 sm:px-6 flex justify-center pointer-events-none">
      <nav className="w-full max-w-6xl bg-white/70 dark:bg-[#18181b]/70 backdrop-blur-xl border border-zinc-200/50 dark:border-zinc-800/50 rounded-2xl shadow-lg shadow-zinc-200/20 dark:shadow-black/20 transition-all duration-300 pointer-events-auto">
          ...
          <div className="hidden md:flex items-center space-x-1 bg-zinc-100/50 dark:bg-zinc-800/50 p-1 rounded-xl">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className={`px-3 lg:px-4 py-1.5 text-sm font-medium rounded-lg transition-all ${
          ...
          <div className="flex items-center space-x-2 sm:space-x-4">
            ...
            {!user ? (
              <button
                ...
                className="flex items-center gap-2 px-2 sm:px-3 py-1.5 text-xs sm:text-sm font-medium bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors"
              >
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                <img src={user.photoURL || ''} alt="Profile" className="w-6 h-6 sm:w-7 sm:h-7 rounded-full border border-zinc-200 dark:border-zinc-800" />
                <button
                  onClick={signOut}
                  className="hidden sm:block px-3 py-1.5 text-sm font-medium text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors"
                >
                  Sign Out
                </button>
              </div>
